...
def create_train_model(x_train, y_train, epochs, lstm_units_1, lstm_units_2, dropout_rate, progress_text):
    model = Sequential()
...
# Function to create and train LSTM model
def create_train_model_predict(x_train, y_train, epochs, lstm_units_1, lstm_units_2, dropout_rate, progress_text):
    model = Sequential()
    model.add(LSTM(lstm_units_1, return_sequences=True, input_shape=(x_train.shape[1], 1)))
    ...
    st.write("Process predicted close price, please wait...")
...
if selected == 'Bursa Efek Indonesia':
    ...
    data_period = st.selectbox("Period", ('1d', '5d', '1mo', '3mo', '6mo', '1y', '2y', '5y', '10y', 'ytd', 'max'))
    ...
    lstm_units_1 = st.number_input('Enter the number of units for the first LSTM layer', value=50, key='lstm_units_1')
    lstm_units_2 = st.number_input('Enter the number of units for the second LSTM layer', value=50, key='lstm_units_2')
    dropout_rate = st.slider('Select the dropout rate', 0.0, 0.5, 0.2, 0.01)
    days_to_predict = 5
    ...
    if st.button('Train Model'):
        ...
        model = create_train_model(x_train, y_train, epochs, lstm_units_1, lstm_units_2, dropout_rate, progress_text)
    ...
    if st.button('Prediction'):
        ...
        # Create and train LSTM model
        model = create_train_model_predict(x_train, y_train, epochs, lstm_units_1, lstm_units_2, dropout_rate, progress_text)
